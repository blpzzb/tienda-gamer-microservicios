package cl.duoc.tienda.categoria.controller;

import cl.duoc.tienda.categoria.exception.ApiExceptionHandler;
import cl.duoc.tienda.categoria.model.Categoria;
import cl.duoc.tienda.categoria.service.CategoriaService;
...
class CategoriaWebLayerTest {
    @Test void controllerDelegatesCrudOperations() {
        CategoriaService service = mock(CategoriaService.class); Categoria item = new Categoria("Perifericos", "Desc"); item.setId(1L);
    ...
    @Test void exceptionHandlerMapsValidationErrors() {
        MethodArgumentNotValidException exception = mock(MethodArgumentNotValidException.class); BindingResult result = mock(BindingResult.class); when(exception.getBindingResult()).thenReturn(result); when(result.getFieldErrors()).thenReturn(List.of(new FieldError("categoria", "nombre", "requerido")));
