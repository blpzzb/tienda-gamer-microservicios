package cl.duoc.tienda.resena.controller;

import cl.duoc.tienda.resena.exception.ApiExceptionHandler;
import cl.duoc.tienda.resena.model.Resena;
import cl.duoc.tienda.resena.service.ResenaService;
...
class ResenaWebLayerTest {
    @Test void controllerDelegatesCrudOperations() {
        ResenaService service = mock(ResenaService.class); Resena item = new Resena(1L, 2L, 5, "Bueno"); item.setId(1L);
        ...
        ResenaController controller = new ResenaController(service);
    ...
    @Test void exceptionHandlerMapsValidationErrors() {
        MethodArgumentNotValidException exception = mock(MethodArgumentNotValidException.class); BindingResult result = mock(BindingResult.class); when(exception.getBindingResult()).thenReturn(result); when(result.getFieldErrors()).thenReturn(List.of(new FieldError("resena", "comentario", "requerido")));
